...
resource "aws_vpc" "main" {
  ...
  tags = {
    Name = var.vpc_name != "" ? var.vpc_name : "${var.project_name}-relay"
  }
}
...
# Security Group for Specific IPs
resource "aws_security_group" "xdr-ips" {
  name_prefix = "${var.project_name}-xdr-ips-sg"
  vpc_id      = aws_vpc.main.id
  ...
  # HTTP access from specific IPs (if needed)
  ingress {
  ...
  tags = {
    Name = "${var.project_name}-xdr-ips-sg"
  }
}
